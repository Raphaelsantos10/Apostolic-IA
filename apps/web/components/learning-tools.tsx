...
export function LessonLearningTools({ lessonId }: Readonly<{ lessonId: string }>) {
  ...
  const submitQuiz = async () => {
    ...
    if (!error && data?.[0]) setResult(data[0] as QuizResult);
  };

  return (
    <section className="learning-tools" aria-label="Ferramentas da aula">
      <div className="learning-actions">
        <button className="button button-secondary" type="button" onClick={toggleComplete}>
          {completed ? "✓ Concluída" : "Marcar como concluída"}
        </button>
        <button className="button button-secondary" type="button" onClick={toggleFavorite}>
      ...
      <button className="button button-secondary" type="button" onClick={saveNote}>Guardar anotação</button>
      ...
      {question && (
        <fieldset className="lesson-quiz">
          ...
          <button className="button button-primary" type="button" onClick={submitQuiz} disabled={selected === null}>
            Corrigir resposta
          </button>
          {result && (
