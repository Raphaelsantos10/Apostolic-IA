...
import v472Styles from "./arena-card-gallery-v472.module.css";
import rarityStyles from "./arena-card-rarity-v7.module.css";

const DECK_STORAGE_KEY = "apostolic-arena-active-deck";
const RARITY_LABEL = { common: "COMUM", rare: "RARA", epic: "ÉPICA", legendary: "LENDÁRIA", champion: "CAMPEÃO" } as const;

function ArenaCard({ card, selected, inDeck, onSelect }: { card: ArenaCatalogCard; selected: boolean; inDeck: boolean; onSelect: () => void }) {
  return <button type="button" className={`${styles.card} ${v472Styles.card} ${rarityStyles.rarityCard} ${rarityStyles[card.rarity]} ${selected ? styles.selected : ""}`} onClick={onSelect} aria-pressed={selected}>
    <span className={styles.faith}>{card.faith}</span>
    {inDeck && <span className={styles.inDeck}>✓ NO DECK</span>}
    <img className={v472Styles.cardArt} src={card.portrait} alt={`Arte da carta ${card.name}`} width={480} height={600} loading="eager" decoding="async" />
    <span className={`${styles.rarity} ${rarityStyles.rarityBand}`}>{RARITY_LABEL[card.rarity]}</span>
...
export function ArenaCardGallery() {
  ...
  return <section className={`${styles.shell} ${v472Styles.shell}`} aria-labelledby="collection-title">
    ...
    <section className={styles.deckBuilder} aria-labelledby="deck-title">
      ...
      <div className={styles.deckSlots}>{Array.from({ length: 8 }, (_, index) => { const card = deck[index]; return card ? <button type="button" className={`${rarityStyles.deckSlot} ${rarityStyles[card.rarity]}`} key={card.id} onClick={() => { setSelectedId(card.id); saveDeck(deckIds.filter((id) => id !== card.id)); setMessage(`${card.name} foi removida do baralho.`); }} title={`Remover ${card.name}`}><img className={v472Styles.deckArt} src={card.portrait} alt={card.name} width={160} height={200} decoding="async"/><b>{card.faith}</b></button> : <span key={`empty-${index}`} aria-label="Espaço vazio">+</span>; })}</div>
      <p className={styles.deckMessage} aria-live="polite">{message}</p>
    </section>
    <div className={styles.layout}>
      <div className={`${styles.grid} ${v472Styles.grid}`}>{ARENA_CARD_CATALOG.map((card) => <ArenaCard key={card.id} card={card} selected={card.id === selected?.id} inDeck={deckIds.includes(card.id)} onSelect={() => setSelectedId(card.id)} />)}</div>
      {selected && <aside className={`${styles.details} ${rarityStyles.details} ${rarityStyles[selected.rarity]}`}><img className={v472Styles.detailsArt} src={selected.portrait} alt="" width={720} height={900} decoding="async"/><p className="eyebrow">#{selected.id} · {RARITY_LABEL[selected.rarity]} · {selected.faith} Fé</p><h3>{selected.name}</h3><p>{selected.description}</p><dl><div><dt>Tipo</dt><dd>{selected.type}</dd></div><div><dt>HP</dt><dd>{selected.hp}</dd></div><div><dt>Dano / DPS</dt><dd>{selected.damage} / {selected.dps}</dd></div><div><dt>Alcance</dt><dd>{selected.range}</dd></div><div><dt>Velocidade</dt><dd>{selected.speed}</dd></div></dl><button type="button" className="button button-primary" onClick={toggleSelected}>{deckIds.includes(selected.id) ? "Remover do baralho" : "Adicionar ao baralho"}</button></aside>}
    </div>
  </section>;
}
