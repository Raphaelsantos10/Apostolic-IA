...
export function BibleGame(){
 ...
 const [feedback,setFeedback]=useState(""),[ready,setReady]=useState(false);
 ...
 return <section className="bible-game"><p className="eyebrow">Aprendizagem, não competição espiritual</p><h1>Desafio bíblico</h1>
 ...
 {question&&<article><h2>{question.prompt}</h2><div className="game-options">{question.options.map((option,index)=><button type="button" key={option} onClick={()=>void answer(index)}>{option}</button>)}</div></article>}
