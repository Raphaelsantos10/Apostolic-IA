...
export async function POST(request: Request) {
  ...
  if (
    ...
    object.metadata?.kind === "apostolic_arena"
  ) {
    if (object.payment_status !== "paid")
      return NextResponse.json({ received: true, pending: true });
    const { error: fulfillError } = await admin.rpc(
      "arena_fulfill_money_purchase",
      {
        p_receipt_id: object.metadata.receipt_id,
        p_checkout_session_id: String(object.id),
        p_payment_intent_id: String(object.payment_intent),
        p_amount_minor: Number(object.amount_total),
    ...
    if (fulfillError) {
      ...
      return NextResponse.json(
        { error: "Compra da Arena não reconciliada." },
        { status: 500 },
      );
    }
  }
  ...
  if (
    (fullRefund || event.type === "charge.dispute.created") &&
    object.payment_intent
  ) {
    const { error: reverseError } = await admin.rpc(
      "arena_reverse_money_purchase",
      {
        p_payment_intent_id: String(object.payment_intent),
        p_reason:
    ...
    if (reverseError) {
      await admin
        ...
        .eq("event_id", event.id);
      return NextResponse.json(
        { error: "Reversão da Arena não reconciliada." },
        { status: 500 },
      );
    }
  }
  ...
  if (
    ...
    if (
      subscription.metadata?.kind === "apostolic_alliance_pro" &&
      ...
      const status =
        event.type === "customer.subscription.deleted"
          ? "canceled"
          : supported.has(subscription.status)
            ? subscription.status
            : "unpaid";
      const { error: allianceError } = await admin.rpc(
        "arena_fulfill_alliance_pro",
        {
          p_intent_id: subscription.metadata.intent_id,
      ...
      if (allianceError) {
        await admin
          ...
          .eq("event_id", event.id);
        return NextResponse.json(
          { error: "Aliança PRO não reconciliada." },
          { status: 500 },
        );
      }
    }
    ...
    if (userId && planCode) {
      ...
      const status =
        event.type === "customer.subscription.deleted"
          ? "canceled"
          : supportedStatus.has(subscription.status)
            ? subscription.status
            : "unpaid";
      ...
      if (subscriptionError) {
        await admin
          ...
          .eq("event_id", event.id);
        return NextResponse.json(
          { error: "Assinatura não atualizada." },
          { status: 500 },
        );
      }
    }
