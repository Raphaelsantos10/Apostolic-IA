...
export default async function ResultPage({ searchParams }: Readonly<{ searchParams: Params }>) {
  ...
  return <><p className="eyebrow">Plano inicial</p><h1>{plan}</h1><p className="auth-message">Avaliação: {params.pontuacao ?? "0"} de 5. Esta indicação não limita os cursos futuros.</p><p><Link className="button button-primary" href="/dashboard">Continuar</Link></p><p><Link href="/onboarding">Refazer onboarding</Link></p></>;
}
